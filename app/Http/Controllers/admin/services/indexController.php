    ...
    public function store(Request $request)
    {
        if (isset($request->serviceTitle) && isset($request->serviceDescription) && isset($request->foto)) {
            if (functionController::resimTurKontrol($request->File("foto")->getClientOriginalExtension())) {
                $image = functionController::imageCreate($request, "services", "foto");
                $imageWebp = functionController::imageCreateWebp($request, "services", "foto");
    ...
    public function destroy(string $id)
    {
        if (isset($id)) {
            ...
            if ($service[0]["title"]) {
                ...
                if ($delete) {
                    return redirect()->back()->with("status", "Servis Başarı ile Silindi.");
                } else {
                    return redirect()->back()->with("status", "Servis ile Silinemedi.");
                }
            }
        }
    }
        ...
        if (isset($id)) {
            ...
            if ($service[0]["title"]) {
                if ($service[0]["isPopular"] == 1) {
                    ...
                    $update = services::where("id", functionController::security($id))->update($all);
                    if ($update) {
                        return redirect()->back()->with("status", "Servis Başarı ile Güncellendi.");
                    } else {
                        return redirect()->back()->with("status", "Servis ile Güncellenemedi.");
                    }
                }else{
                    $all=["isPopular"=>1];
                    $update = services::where("id", functionController::security($id))->update($all);
